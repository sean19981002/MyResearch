...
configs.read('v1_tweepy/config.ini')
...
for status in tweepy.Cursor(api.user_timeline, screen_name='POTUS',count=120).items(1000):
    s = json.dumps(status)
    s = json.loads(s)
    json_data[count] = s
    count += 1
    
with open("v1_tweepy/Biden.json", "w") as f:
